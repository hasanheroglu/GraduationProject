...
public class KillActivity : Activity
{
    private static Activity _instance;

    public KillActivity(ActivityType type) : base(type)
    {
    ...
    {
        if(_instance == null) _instance = new KillActivity(ActivityType.Kill);
        return _instance;
    }
    ...
    private void FindWeapon(Responsible responsible)
    {
        var interactableObjects = Physics.OverlapSphere(responsible.gameObject.transform.position, 100f);
        ...
        foreach (var interactableObject in interactableObjects)
        {
            ...
            var equipable = interactable.gameObject.GetComponent<Equipable>();

            if (equipable == null || equipable.type != EquipableType.Weapon) continue;
            
            var method = interactable.FindAllowedAction(responsible, Type);
            if (method == null) continue;
			
            var coroutineInfo = new JobInfo(responsible, equipable, equipable.GetType().GetMethod("Equip"), new object[] {responsible});
            UIManager.SetInteractionAction(coroutineInfo);
            return;
        }
    }
